...
To enable logging in as a `pi` user makes the following changes after installing DWagent

1. Change ownership of the agent folder

    ```bash
    sudo chown -R pi:pi /usr/share/dwagent
    ```

2. Modify the service file (e.g. /etc/systemd/system/dwagent.service) to include:
...
3. Reload and restart:
    ...
    ```bash
    sudo systemctl daemon-reload && sudo systemctl restart dwagent
    ```
